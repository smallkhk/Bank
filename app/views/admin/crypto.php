...
$e0 = $edit ?? ['id' => '', 'symbol' => '', 'name' => '', 'decimals' => 8, 'price' => 0, 'trade_fee_bps' => 100, 'min_trade' => 100, 'volatility_bps' => 0, 'status' => 'active', 'sort_order' => 0];
?>
...
<section class="card"><div class="table-wrap"><table class="table">
  ...
  <tbody><?php foreach ($assets as $a): $ch = C::changeBps($a); ?>
    <tr><td><strong><?= e($a['symbol']) ?></strong> <span class="muted"><?= e($a['name']) ?></span><div class="muted small"><?= (int) $a['decimals'] ?> decimals<?= (int) $a['volatility_bps'] ? ' · simulator ±' . number_format($a['volatility_bps'] / 100, 2) . '%' : '' ?></div></td>
...
<section class="card" id="asset-form">
  ...
  <form method="post" action="<?= e(url('admin/crypto')) ?>" class="form grid-3"><?= csrf_field() ?><input type="hidden" name="id" value="<?= e($e0['id']) ?>">
    ...
    <?php if (!$edit): ?><label>Initial price <input name="price" inputmode="decimal" required placeholder="0.00"></label><?php endif; ?>
