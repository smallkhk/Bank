...
<div class="page-head"><h1>Crypto <span class="badge badge-warning">Simulated</span></h1></div>
...
<section class="card narrow-wide">
  ...
  <form method="post" action="<?= e(url('crypto/acknowledge')) ?>" class="form"><?= csrf_field() ?>
    <label class="check"><input type="checkbox" name="accept" value="1" required> I understand these are simulated assets and that I can lose money used to buy them.</label>
    <div><button class="btn btn-primary">Continue</button></div></form>
</section>
<?php endif; ?>

<div class="stat-grid">
  <div class="stat stat-primary"><span>Portfolio value</span><strong><?= e(money($t['value'])) ?></strong><small>Simulated holdings at current prices</small></div>
  ...
  <?php foreach ($assets as $a): $ch = C::changeBps($a); ?>
    <a class="list-row link-row" href="<?= e(url('crypto/' . $a['symbol'])) ?>">
      ...
      <div class="right"><strong><?= e(money((int) $a['price'])) ?></strong>
        <div class="small <?= $ch >= 0 ? 'pos' : 'neg' ?>"><?= $ch >= 0 ? '▲' : '▼' ?> <?= number_format(abs($ch) / 100, 2) ?>%<?= $a['status'] === 'halted' ? ' · <span class="badge badge-warning">Halted</span>' : '' ?></div></div>
    </a>
  <?php endforeach; ?>
</section>
<section class="card">
  <h2>Your holdings</h2>
  <?php if (!$portfolio['rows']): ?><p class="empty">You don't hold any simulated assets.</p><?php else: ?>
