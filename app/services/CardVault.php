...
final class CardVault
{
    private static function key(string $purpose): string
    {
        ...
        if (strlen($appKey) < 32 || str_contains($appKey, 'CHANGE-ME')) {
            throw new \RuntimeException('Set a random app.key of at least 32 characters in config/config.php before issuing cards.');
        }
    ...
    public static function encrypt(string $plain): string
    {
        $iv = random_bytes(12);
        $tag = '';
        $cipher = openssl_encrypt($plain, 'aes-256-gcm', self::key('card-data'), OPENSSL_RAW_DATA, $iv, $tag);
    ...
    public static function decrypt(string $stored): string
    {
        ...
        $plain = openssl_decrypt(substr($raw, 28), 'aes-256-gcm', self::key('card-data'), OPENSSL_RAW_DATA, substr($raw, 0, 12), substr($raw, 12, 16));
        if ($plain === false) {
            throw new \RuntimeException('Card data could not be decrypted (was app.key changed?).');
        }
        return $plain;
    }
